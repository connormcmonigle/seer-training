...
def to_image(x):
  ...
  x = x.reshape(32,8,  8,8,  2,1,  8,8)

  # height ---- width
  x = x.permute(0,5,6,2,  1,4,7,3)
  x = (x.reshape(32*1*8*8, 8*2*8*8) * 6.0).abs().sigmoid()
  return x
...
def plot_input_weights_image(x, name, dst):
  ...
  for j in range(1, 32):
    dst.axhline(y=1*8*8*j-1, **line_options)
...
def main():
  ...
  w = to_image(nnue.p_white_affine.virtual_weight())
  b = to_image(nnue.p_black_affine.virtual_weight())
  ...
  plt.savefig(os.path.join(cfg.visual_directory, 'p_feature_transformer.png'))
